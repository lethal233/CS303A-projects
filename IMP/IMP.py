...
def get_RR(partition, end_time, model, graph, N):
    ...
    if model == 'IC':
        while count < partition:
            v = random.randint(1, N)
            rr_tmp.append(get_RR_IC([v], graph))
            count += 1
    else:
        while count < partition:
            v = random.randint(1, N)
            rr_tmp.append(get_RR_LT([v], graph))
            count += 1
    return rr_tmp
...
if __name__ == '__main__':
    ...
    parser.add_argument('-k', '--pre_size', type=int, default=100)
    ...
    # end_time = start_time + time_limit - 5

    if k <= 50:
        end_time = start_time + time_limit * 0.4
    else:
        end_time = start_time + time_limit * 0.3
